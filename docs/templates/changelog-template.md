# CHANGELOG

> **Template.** Copy this over `docs/CHANGELOG.md` in your own project:
> `cp docs/templates/changelog-template.md docs/CHANGELOG.md`.
> The kit's own `docs/CHANGELOG.md` is a *live* file — read it for filled-in entries.
>
> Append-only record of delivered work. One entry per completed task, newest on top.
> This file is **not** read in full at session start — grep it when you need history.
> Loosely follows [Keep a Changelog](https://keepachangelog.com); adapt as you like.

<!-- New entries go directly below this line. -->

## <date> — <task title> ✅
- **Commit(s):** `<hash>`
- **What:** <1–3 lines>
- **Decisions touched:** <link to `decisions/NNNN-…` or "none">
- **Verified:** <how — unit tests / e2e / manual, with the result>
- **Left for human:** <anything not auto-verified, or "none">
